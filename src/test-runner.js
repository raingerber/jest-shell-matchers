    ...
    default:
      ...
      if (result.keys.length) {
        expandedMessages.push([
          ``,
          `Non-matching keys: ${result.keys.map(key => `"${key}"`).join(', ')}`
        ])
      }
      expected = util.inspect(expected, inspectOptions)
      received = util.inspect(received, inspectOptions)
  }
...
function createMessageFn (options) {
  ...
  // if (this.expand) {
  //   expandedMessages = options.expandedMessages || expandedMessages
  // }
